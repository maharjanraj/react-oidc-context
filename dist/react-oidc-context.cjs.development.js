...
var hasAuthParams = function hasAuthParams(location) {
  ...
  if ((searchParams.get("code") || searchParams.get("access_token") || searchParams.get("error")) && searchParams.get("state")) {
    return true;
  }
...
var loginError = /*#__PURE__*/normalizeErrorFn("Login failed");

var userManagerContextKeys = ["clearStaleState", "signinPopup", "signinSilent", "signinRedirect", "querySessionStatus", "revokeAccessToken", "startSilentRenew", "stopSilentRenew"];
...
var defaultUserManagerImpl = typeof window === "undefined" ? null : oidcClient.UserManager;
/**
...
var AuthProvider = function AuthProvider(props) {
  var children = props.children,
      ...
      userManagerProps = _objectWithoutPropertiesLoose(props, ["children", "onSigninCallback", "skipSigninCallback", "onRemoveUser", "onSignoutRedirect", "onSignoutPopup", "implementation"]);

  var _React$useState = React__default["default"].useState(function () {
    return UserManagerImpl && new UserManagerImpl(userManagerProps);
  }),
      userManager = _React$useState[0];

  var _React$useReducer = React__default["default"].useReducer(reducer, initialAuthState),
      state = _React$useReducer[0],
      dispatch = _React$useReducer[1];

  var userManagerContext = React__default["default"].useMemo(function () {
    var _userManager$settings;

    return _extends({
      settings: (_userManager$settings = userManager == null ? void 0 : userManager.settings) != null ? _userManager$settings : {}
    }, Object.fromEntries(userManagerContextKeys.map(function (key) {
      return [key, userManager ? userManager[key].bind(userManager) : unsupportedEnvironment(key)];
    })));
  }, [userManager]);
      ...
      return runtime_1.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 4:
              _user = _context.sent;
              onSigninCallback && onSigninCallback(_user);
  ...
  }, [userManager, skipSigninCallback, onSigninCallback]); // register to userManager events

  React__default["default"].useEffect(function () {
    if (!userManager) return; // event UserLoaded (e.g. initial load, silent renew success)
    ...
    userManager.events.addUserLoaded(handleUserLoaded); // event UserUnloaded (e.g. userManager.removeUser)
    ...
    userManager.events.addUserUnloaded(handleUserUnloaded); // event SilentRenewError (silent renew error)
  ...
  var removeUser = React__default["default"].useMemo(function () {
    return userManager ? function () {
      return userManager.removeUser().then(onRemoveUser);
    } : unsupportedEnvironment("removeUser");
  }, [userManager, onRemoveUser]);
  var signoutRedirect = React__default["default"].useMemo(function () {
    return userManager ? function (args) {
      return userManager.signoutRedirect(args).then(onSignoutRedirect);
    } : unsupportedEnvironment("signoutRedirect");
  }, [userManager, onSignoutRedirect]);
  var signoutPopup = React__default["default"].useMemo(function () {
    return userManager ? function (args) {
      return userManager.signoutPopup(args).then(onSignoutPopup);
  ...
  return React__default["default"].createElement(AuthContext.Provider, {
    value: _extends({}, state, userManagerContext, {
...
//# sourceMappingURL=react-oidc-context.cjs.development.js.map
